...
class evScriptOptimizer {
    ...
    function init() {
		...
		/*
		$is_logged_in = true;
        if (empty($_COOKIE[str_replace('wordpress_', 'wordpress_logged_in_', AUTH_COOKIE)]))
            $is_logged_in = false;
        else {
            $cookie_elements = explode('|', $_COOKIE[str_replace('wordpress_', 'wordpress_logged_in_', AUTH_COOKIE)]);
            if (count($cookie_elements) !== 3)
                $is_logged_in = false;
            else {
                if ($cookie_elements[1] < time())
                    $is_logged_in = false;
                else {
                    $userdata = WP_User::get_data_by('login', $cookie_elements[0]);
                    if (!$userdata)
                        $is_logged_in = false;
                    else {
						$user = new WP_User;
						...
						if (!$user)
							$is_logged_in = false;
						unset ($user, $userdata, $cookie_elements);
					}    
				}    
			}    
		}
