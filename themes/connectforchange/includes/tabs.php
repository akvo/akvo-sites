...
global $tabOptions;
        ...
        $aExclude=get_category_ids(array('vacancies','thematic-partner','alliance-members','none','uncategorized'));
        ...
        if($tabOptions['doquery']==true){
            ...
            if((!isset($tabOptions['onlyupdates']) || $tabOptions['onlyupdates']==0) && (count($aCategoryIDs)>0 || isset($tabOptions['country']))){
                ...
                if(isset($tabOptions['country'])){
                    $aTagIDs = get_tag_ids(array(str_replace(array(' ',','),array('-',''),$tabOptions['country'])));
                    $args['tag__in']=join(',',$aTagIDs);
                }
                //get posts for category
                $aPosts = $wp_query->query($args);
            }
//            akvo_debug_dump($tabOptions);
//            akvo_debug_dump($args);
