...
if ( ! function_exists( 'get_thumbnail' ) ){
	...
	//var_dump($blog_id);
		foreach($thumb_array AS $k=>$v){
			$thumb_array[$k]=str_replace('/wp-content/blogs.dir/'.$blog_id.'/files','/files',$v);
                }
		return $thumb_array;
	}
}
...
//			var_dump($new_method_thumb);
//            if ( is_wp_error( $new_method_thumb ) ) $new_method_thumb = '';
		//}
        $new_method_thumb = '/wp-content/plugins/akvo-site-config/classes/thumb.php?src='.$thumbnail.'&w='.$width.'&h='.$height.'&q=100';
		if ($forstyle === false) {
